  ...
  describe '配送先情報の保存' do
    ...
    context '配送先情報の保存ができないとき' do
      ...
      it '郵便番号が空だと保存できないこと' do
        ...
        @order_addresse.valid?
        expect(@order_addresse.errors.full_messages).to include("Post code can't be blank", "Post code is invalid. Include hyphen(-)")
      end
      it '郵便番号にハイフンがないと保存できないこと' do
        @order_addresse.post_code = 1_234_567
        @order_addresse.valid?
        expect(@order_addresse.errors.full_messages).to include('Post code is invalid. Include hyphen(-)')
      end
      it '都道府県が「---」だと保存できないこと' do
        @order_addresse.prefecture_id = 0
        @order_addresse.valid?
        expect(@order_addresse.errors.full_messages).to include("Prefecture can't be blank")
      end
      it '都道府県が空だと保存できないこと' do
        @order_addresse.prefecture_id = nil
        @order_addresse.valid?
        expect(@order_addresse.errors.full_messages).to include("Prefecture can't be blank")
      end
      ...
      it '電話番号が空だと保存できないこと' do
        @order_addresse.phone_numher = nil
        @order_addresse.valid?
        expect(@order_addresse.errors.full_messages).to include("Phone numher can't be blank")
      end
      it '電話番号にハイフンがあると保存できないこと' do
        @order_addresse.phone_numher = '123 - 1234 - 1234'
        @order_addresse.valid?
        expect(@order_addresse.errors.full_messages).to include('Phone numher is invalid')
      end
